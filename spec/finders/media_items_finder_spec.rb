...
RSpec.describe MediaItemsFinder do
  describe "#call" do
    ...
    context "with status filter" do
      it "filters by published status" do
        result = described_class.new(status: "published").call
      ...
      it "filters by draft status" do
        result = described_class.new(status: "draft").call
    ...
    context "with media_type filter" do
      it "filters by image type" do
        result = described_class.new(media_type: "image").call
      ...
      it "filters by video type" do
        result = described_class.new(media_type: "video").call
    ...
    context "with year filter" do
      it "filters by year" do
        result = described_class.new(year: 2023).call
    ...
    context "with search query" do
      it "filters by title" do
        result = described_class.new(q: "Sunset").call
    ...
    context "with combined filters" do
      ...
      it "returns empty when no items match all filters" do
        result = described_class.new(status: "draft", media_type: "video").call
    ...
    context "with blank filter values" do
      it "ignores blank status" do
        result = described_class.new(status: "").call

        expect(result.count).to eq(4)
      end

      it "ignores blank media_type" do
        result = described_class.new(media_type: nil).call
